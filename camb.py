...
def set_params(cp=None, verbose=False, **params):
    ...
    * :meth:`.nonlinear.Halofit.set_params`
    ...
    # Set grey body parameters before set_cosmology (which may iterate on H0)
    # Always set grey body params unconditionally on every set_params call
    cp.use_grey_body_radiation = int(params.get("use_grey_body_radiation", 1))
    cp.transition_redshift = float(params.get("transition_redshift", 17.0))
    used_params.add("use_grey_body_radiation")
    used_params.add("transition_redshift")
    for gb_param in ["custom_emissivity", "epsilon_gamma", "z_gamma_inj", "dz_gamma", "grey_body_pad"]:
        if gb_param in params:
            setattr(cp, gb_param, params[gb_param])
            used_params.add(gb_param)
