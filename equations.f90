    ...
    real(dl) :: grhog_eff, current_z

    call this%CP%DarkEnergy%BackgroundDensityAndPressure(this%grhov, a, grhov_t)

    !  8*pi*G*rho*a**4.
    grhoa2 = this%grho_no_de(a) +  grhov_t * a**2
    if (this%CP%use_grey_body_radiation /= 0) then
        current_z = 1.0_dl/a - 1.0_dl
        if (current_z > this%CP%transition_redshift) then
            grhog_eff = this%CP%custom_emissivity * this%grhog
            grhoa2 = grhoa2 - this%grhog + grhog_eff
        endif
    endif
    if (grhoa2 <= 0) then
    ...
    real(dl) Rp15,tau,x,x2,x3,om,omtau, &
        Rc,Rb,Rv,Rg,grhonu,chi,grhog_ic
    real(dl) k,k2
    ...
    ! Emissivity correction: at IC epoch z >> z_transition, emissivity is active
    if (CP%use_grey_body_radiation /= 0) then
        grhog_ic = CP%custom_emissivity * State%grhog
    else
        grhog_ic = State%grhog
    end if

    om = (State%grhob+State%grhoc)/sqrt(3*(grhog_ic+grhonu))
    omtau=om*tau
    Rv=grhonu/(grhonu+grhog_ic)
    ...
    implicit none
    real(dl) bigR,tau,x,aj3r,elec, pir, rhomass, grhog_ic
    integer l
    type(EvolutionVars) EV
    ...
    ! Emissivity correction for tensor ICs
    if (CP%use_grey_body_radiation /= 0) then
        grhog_ic = CP%custom_emissivity * State%grhog
    else
        grhog_ic = State%grhog
    ...
    omtau = tau*(State%grhob+State%grhoc)/sqrt(3*(grhog_ic+rhomass+State%grhornomass))
    a=tau*State%adotrad*(1+omtau/4)

    if (DoTensorNeutrinos) then
        bigR = (rhomass+State%grhornomass)/(rhomass+State%grhornomass+grhog_ic)
    else
        bigR = 0._dl
    end if
    ...
    implicit none
    real(dl) bigR,Rc,tau,x,pir,grhog_ic
    type(EvolutionVars) EV
    real(dl) k,k2 ,a, omtau
    real(dl) yv(EV%nvarv)
    ...
    ! Emissivity correction for vector ICs
    if (CP%use_grey_body_radiation /= 0) then
        grhog_ic = CP%custom_emissivity * State%grhog
    else
        grhog_ic = State%grhog
    ...
    omtau = tau*(State%grhob+State%grhoc)/sqrt(3*(grhog_ic+State%grhornomass))

    a=tau*State%adotrad*(1+omtau/4)

    x=k*tau

    bigR = (State%grhornomass)/(State%grhornomass+grhog_ic)
    Rc=CP%omch2/(CP%omch2+CP%ombh2)
    ...
    real(dl) dgq,grhob_t,grhor_t,grhoc_t,grhog_t,grhov_t,grhonu_t,sigma,polter,z,f
    ...
    real(dl) a,a2,clxc,clxb,vb,clxg,qg,pig,clxr,qr,pir
    real(dl) E2, dopacity
    ...
    if (CP%use_grey_body_radiation /= 0 .and. (1.0_dl/a - 1.0_dl) > CP%transition_redshift) then
        grhog_t = CP%custom_emissivity * grhog_t
    end if
    ...
    real(dl)  grhob_t,grhor_t,grhoc_t,grhog_t,grhov_t,polter,z,f
    ...
    real(dl) k,k2,a,a2,grhog_t, grhor_t,z,f
