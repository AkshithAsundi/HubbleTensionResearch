...
10      if (this%CP%Omnuh2 < 1.e-7_dl) this%CP%Omnuh2 = 0
        ...
        ! z_eq corrected for emissivity (z_eq ~ 3400 >> z_transition, emissivity is active)
        if (this%CP%use_grey_body_radiation /= 0) then
            this%z_eq = (this%grhob+this%grhoc)/(this%CP%custom_emissivity*this%grhog+this%grhornomass+sum(this%grhormass(1:this%CP%Nu_mass_eigenstates))) -1
        else
    ...
    astar = 1/(1+zstar)
    atol = 1e-6
    write(*,*) "CosmomcTheta: use_grey_body=", this%CP%use_grey_body_radiation, " emiss=", this%CP%custom_emissivity
    ...
    class(CAMBdata) :: this
    real(dl) dsound_da_exact,a,R,cs,grhog_eff,current_z

    grhog_eff = this%grhog
    if (this%CP%use_grey_body_radiation /= 0) then
        current_z = 1.0_dl/a - 1.0_dl
        if (current_z > this%CP%transition_redshift) grhog_eff = this%CP%custom_emissivity * this%grhog
    endif
    R = 3*this%grhob*a / (4*grhog_eff)
    cs=1.0d0/sqrt(3*(1+R))
    ...
    real(dl) dsound_da_approx,a,R,cs,grhog_eff,current_z
